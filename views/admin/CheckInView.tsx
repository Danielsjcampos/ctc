...
const CheckInView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', table: 'club_sessions' }, () => {
        fetchActiveSessions();
      })
      .subscribe();
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="glass p-10 rounded-[40px] border-white/5 space-y-8">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-black uppercase tracking-tighter">Status das <span className="text-blue-500">Raias</span></h3>
            ...
            <div className="grid md:grid-cols-2 gap-4">
                ...
                <div key={session.id} className="p-6 bg-white/5 border border-white/10 rounded-[32px] flex items-center justify-between group hover:border-blue-600/30 transition-all">
                   <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-gray-500 uppercase font-black">{session.firearm_model} • {new Date(session.check_in_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}h</span>
                      </div>
                   </div>
                   <button 
                    onClick={() => setCheckoutModal(session)}
                    className="bg-white/5 hover:bg-red-600 text-gray-400 hover:text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
                   >
                     Finalizar
                   </button>
                </div>
              ...
              {activeSessions.length === 0 && (
                <div className="col-span-2 py-20 text-center border-2 border-dashed border-white/5 rounded-[40px]">
                  ...
                  <p className="text-gray-600 text-[10px] font-black uppercase tracking-widest">Nenhum atirador em pista no momento</p>
                </div>
              )}
            </div>
          </div>
